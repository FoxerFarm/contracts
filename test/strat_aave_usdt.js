...
describe("ThenaDepositor", async () => {
    ...
    let tester = (await ethers.getSigners())[3];
    let tester2 = (await ethers.getSigners())[4];
    ...
    beforeEach(async () => {
        ...
        const FoxerStrategyColend01Factory = await ethers.getContractFactory("FoxerStrategyColend01");
        strategy = await upgrades.deployProxy(
            FoxerStrategyColend01Factory,
            [
                admin.address,
        ...
        const FoxerVaultFactory = await ethers.getContractFactory("FoxerVault");
        vault = await upgrades.deployProxy(
            FoxerVaultFactory,
            [
                //function initialize(address _owner, address _wbtc, address _strategy, string memory _name, string memory _symbol, uint256 _approvalDelay) public initializer {
                admin.address,
                process.env.BTC_ADDRESS,
                strategy.target,
                "Foxer USDT",
